...
#pragma omp parallel for private(i) schedule(dynamic)								  
									  for (i = 0; i<VN; i++)
										  H[i] = hidden(V_denoising[i], H[i], t_Wh, VD, HD);
...
#pragma omp parallel for private(j) schedule(dynamic) 
										for (j = 0; j < VD; j++)  
											d_r[j] = (-(1-R[i][j] * R[i][j]) * (V[i][j] - R[i][j]));
											  ...
											  clear_vector(temp, HD);
                                             for (j = 0; j < HD; j++) {     
                                                   for (k = 0; k < VD; k++)
                                                           temp[j] = temp[j] + Wr[j][k] * d_r[k];
												   												  }
#pragma omp parallel for private(j) schedule(dynamic)        
											 for (j = 0; j < HD; j++)  d_h[j] = (temp[j] + B*(((1 - p) / (1 - p_hat[j])) - (p / p_hat[j]))) * (1 - H[i][j] * H[i][j]);
...
#pragma omp parallel for private(j) schedule(dynamic) 
 for (i = 0; i<VN; i++)  V_denoising[i][VD] = 1.0;
while(errorck>= eck){
start = clock(); 	 
ertemp=e_all;
//パラメータ//
p=setting[0];//Sparseの目標値
A=setting[1];//L2ノルムの強さ
B=setting[2];//Sparseの強さ
eck=setting[3];//収束条件
denoising_rate=setting[4];//noiseのパセント
...
BP(V,V_denoising,Wh,Wr,H,R,VN,VD,HD,eck,p,A,B,step);//BP():1回の訓練
